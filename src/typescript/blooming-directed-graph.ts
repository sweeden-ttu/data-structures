/**
 * Blooming-directed-graph (LangGraph namespace): each repository under ~/projects (except data-structures) is a node.
 *
 * A blooming-directed-graph is a directed graph (nodes = repos, edges = directed links to github_actions).
 * The term is distinct from a generic "blooming graph". Interacts with GitHub Actions API.
 * Usage: set GITHUB_TOKEN for API calls. Run with: npx ts-node blooming-directed-graph.ts (or compile and node).
 */
...
/** LangGraph namespace: graph node type */
...
/** LangGraph namespace: graph edge type */
export interface Edge {
  from: string;
  to: string;
}

/** Concrete namespace: LangGraph (graph structures) */
export namespace LangGraph {
  export type { Node, Edge };
  export { BloomingDirectedGraph };
}
